...
import com.zyao.zutils.LoggerManager;
 ...
 * Description: 自定义日志功能
 * Author: Zyao89
 * Time: 2016/7/19 15:32
 */
public class LoggerManagerImpl implements LoggerManager
{
    private static final String DEFAULT_TAG = "LoggerManager";
    /**
     ...
     */
    private final LoggerSettings settings = new LoggerSettings();
    /**
    ...
    public static void registerInstance ()
    {
        if (instance == null)
        {
            synchronized (LoggerManager.class)
            {
                if (instance == null)
                {
                    instance = new LoggerManagerImpl();
                }
            }
        }
        Z.Ext.setLoggerManager(instance);
    }

    @Override
    public LoggerManager t (String tag)
    {
        return t(tag, settings.getMethodCount());
    }

    @Override
    public LoggerManager t (int methodCount)
    {
        return t(null, methodCount);
    }

    @Override
    public LoggerManager t (String tag, int methodCount)
    {
        if (tag != null)
        {
            localTag.set(tag);
    ...
    @Override
    public void openStreamPrint (String filePath)
    {
        if (filePath != null)
        {
            LogReader.startCatchLog(Z.appInfo().getAppPackageName(), filePath);
        }
    }

    @Override
    public void closeStreamPrint ()
    {
    ...
    private int getStackOffset (StackTraceElement[] trace)
    {
        for (int i = MIN_STACK_OFFSET; i < trace.length; i++)
        {
            ...
            if (!name.equals(LoggerManagerImpl.class.getName()) && !name.equals(LoggerManager.class.getName()))
            {
                return --i;
            }
        }
