...
import com.zyao.zcore.debug.DebugFragmentRecord;
import com.zyao.zcore.debug.DebugHierarchyViewContainer;
import com.zyao.zcore.helper.FragmentResultRecord;
import com.zyao.zcore.helper.OnEnterAnimEndListener;
import com.zyao.zcore.helper.OnFragmentDestoryViewListener;
...
public class Fragmentation
{
    ...
    void startWithPop (FragmentManager fragmentManager, SupportFragment from, SupportFragment to)
    {
        ...
        String toName = to.getClass().getName();
        FragmentTransaction ft = fragmentManager.beginTransaction().setTransition(FragmentTransaction.TRANSIT_FRAGMENT_OPEN).add(from.getContainerId(), to, toName).addToBackStack(toName);
        ...
        for (int i = fragmentList.size() - 1; i >= 0; i--)
        {
            ...
            if (fragment instanceof SupportFragment)
            {
                ...
                if (!flag)
                {
                    ...
                    if (args == null || !args.containsKey(ARG_RESULT_RECORD))
                    {
                        break;
                    }
    ...
    void showFragmentStackHierarchyView ()
    {
        ...
        new AlertDialog.Builder(mActivity).setTitle("栈视图").setView(container).setPositiveButton("关闭", null).setCancelable(true).show();
    }
