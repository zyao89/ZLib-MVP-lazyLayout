package com.zyao.views.creditsesameringview;
 ...
 * 新版芝麻信用圆环View
 */
public class ZCreditSesameView extends View
{
    //mLogo
    private String mLogo = "CRAZY";

    //最大角度
    private final static float MAX_ANGLE = 210f;

    // 圆环的信用等级文本
    private String[] mSesameStr = new String[]{"100", "200", "300", "400", "500", "600", "700", "800", "900", "950", "1000"};
    ...
    //信用等级
    private String mSesameLevel = "";

    //评估时间
    private String mEvaluationTime = "统计时间:" + getCurrentTime();
    ...
    public ZCreditSesameView (Context context)
    {

        this(context, null);
    }

    public ZCreditSesameView (Context context, AttributeSet attrs)
    {

        this(context, attrs, 0);
    }

    public ZCreditSesameView (Context context, AttributeSet attrs, int defStyleAttr)
    {
    ...
    private void init ()
    {
        ...
        mMiddleArcPaint.setStrokeWidth(dp2px(2));
        ...
        mInnerArcPaint.setStrokeWidth(dp2px(30));
        ...
        mCalibrationTextPaint.setTextSize(dp2px(10));
...
//        bitmap = BitmapFactory.decodeResource(getResources(), R.drawable.ic_circle);
    ...
    private Bitmap getCirclePoint()
    {
        ...
        canvas.drawCircle(center, center, dp2px(4), paint);
        return bitmap;
    }
    ...
    protected void onSizeChanged (int w, int h, int oldw, int oldh)
    {
        ...
        //外层进度画笔
        mArcProgressPaint.setStrokeWidth(width / 94);
    }
    ...
    private void drawCalibrationAndText (Canvas canvas)
    {
        ...
        for (int i = 1; i < (mSesameStr.length + 1); i++)
        {
//            if (i % 2 != 0)//TODO 隔一个画一条线
            {
                canvas.drawLine(radius, startDst, radius, endDst, mCalibrationPaint);
            }
            // 测量文本的长度
            float textLen = mCalibrationTextPaint.measureText(mSesameStr[i - 1]);
            canvas.drawText(mSesameStr[i - 1], radius - textLen / 2, endDst + 40, mCalibrationTextPaint);
    ...
    public void setSesameLevel (String text)
    {
        mSesameLevel = text;
        postInvalidate();
    }

    public void setSesameLogo (String text)
    {
        mLogo = text;
        postInvalidate();
    }

    public void setSesameStr (String[] str)
    {
        mSesameStr = str;
        postInvalidate();
    }
    ...
    public void setSesameMaxValues (int minValues, int maxValues)
    {
        ...
        {
            strings[j] = String.valueOf(minValues + i * j);
        }
        setSesameStr(strings);
    }
    ...
    public int getCurrSesameValues()
    {
        return mMaxNum;
    }
    ...
    public void setSesameValues (int values)
    {
        if (values <= 350)
        {
            ...
            mTotalAngle = 0f;
            mSesameLevel = "信用较差";
            mEvaluationTime = "评估时间:" + getCurrentTime();
        }
        else if (values <= 550)
        {
            mMaxNum = values;
