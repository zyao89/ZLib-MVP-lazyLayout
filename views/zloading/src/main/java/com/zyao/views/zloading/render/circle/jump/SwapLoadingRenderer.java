/**
 * Title: SwapLoadingRenderer.java
 * Package: com.zyao.views.zloading.render.circle
 * Description: 本内容仅Zyao89持有，转载请标注。
 * Author: Zyao89
 * Date: 2016/9/18
 */
package com.zyao.views.zloading.render.circle.jump;
...
/**
 * Class: SwapLoadingRenderer
 * Description: TODO 功能描述...
 * Author: Zyao89
 * Time: 2016/9/18 19:32
 */
public class SwapLoadingRenderer extends LoadingRenderer
{
    ...
    private static final float DEFAULT_HEIGHT = 15.0f * 8;
    ...
    protected void draw (Canvas canvas)
    {
             ...
             i++)
        {
            if (i == mSwapIndex)
            {
                ...
                canvas.drawCircle(mBallRadius * (i * 2 + 1) + mBallSideOffsets + i * mBallInterval + mSwapBallCenterX, mBallCenterY - mSwapBallCenterY, mBallRadius, mPaint);
            }
            else if (i == (mSwapIndex + 1) % mBallCount)
            {
                mPaint.setStyle(Paint.Style.STROKE);
                canvas.drawCircle(mBallRadius * (i * 2 + 1) + mBallSideOffsets + i * mBallInterval - mSwapBallCenterX, mBallCenterY + mSwapBallCenterY, mBallRadius - mStrokeWidth / 2, mPaint);
            }
            else
            {
                mPaint.setStyle(Paint.Style.STROKE);
                canvas.drawCircle(mBallRadius * (i * 2 + 1) + mBallSideOffsets + i * mBallInterval, mBallCenterY, mBallRadius - mStrokeWidth / 2, mPaint);
            }
        }
    ...
    protected void computeRender (float renderProgress)
    {
        ...
        mSwapBallCenterX = mSwapIndex == mBallCount - 1 ? -swapTraceProgress * swapTraceRadius * 2 : swapTraceProgress * swapTraceRadius * 2;

        // (r, r) as the origin of coordinates
        float xCoordinate = mSwapIndex == mBallCount - 1 ? mSwapBallCenterX + swapTraceRadius : mSwapBallCenterX - swapTraceRadius;

        // Calculate the coordinate y
        mSwapBallCenterY = (float) (mSwapIndex % 2 == 0 && mSwapIndex != mBallCount - 1 ? Math.sqrt(Math.pow(swapTraceRadius, 2.0f) - Math.pow(xCoordinate, 2.0f)) : -Math.sqrt(Math.pow(swapTraceRadius, 2.0f) - Math.pow(xCoordinate, 2.0f)));
